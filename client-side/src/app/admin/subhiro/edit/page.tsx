...
import { useUpdateSubhiroMutation } from "@/store/features/moderator/moderatorApiSlice";
import { notFound, useRouter } from "next/navigation";
import { Spinner } from "flowbite-react";
import DynamicInput from "@/components/DynamicInput";
import { useFetchSubhiroQuery } from "@/store/features/subhiro/subhiroApiSlice";
...
const NewSubhiro = ({
    ...
    const { id: subhiroId } = use(searchParams);
    const {
        data: targetSubhiroData,
        isSuccess: isTargetSubhiroDataSuccess,
        isFetching: isTargetSubhiroDataFetching,
    } = useFetchSubhiroQuery({ subhiroId });

    if (!subhiroId) {
        return notFound();
    }

    const [updateSubhiro, { isLoading: isUpdateSubhiroLoading }] =
    ...
    // Load Subhiro Data
    useEffect(() => {
        if (isTargetSubhiroDataFetching) return;
        if (!isTargetSubhiroDataSuccess) {
            toast.error("Target Subhiro Not Found");
            router.push("/admin/subhiro/new");
        }

        setSubhiroValues({
            displayname: targetSubhiroData.displayname,
            description: targetSubhiroData.description,
            profile_picture: targetSubhiroData.profile_picture,
            banner: targetSubhiroData.banner,
            rules: targetSubhiroData.rules,
        });
    }, [isTargetSubhiroDataSuccess, isTargetSubhiroDataFetching]);
    ...
    const handleUpdateSubhiro = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            router.push(`/subhiro/${subhiroId}`);
        } catch (error: any) {
            toast.error(error?.data?.message ?? "Failed to Update Subhiro");
            setIsUploading(false);
        }
    };
    ...
    return (
        <div>
            ...
            {/* New Subhiro */}
            <form onSubmit={handleUpdateSubhiro} className="space-y-5">
                ...
                {formStage === 4 && (
                    <div className="rounded-xl shadow-lg border overflow-hidden max-w-xl mx-auto">
                        ...
                        <div className="h-40 w-full bg-gray-200">
                            {bannerFile.length > 0 || subhiroValues.banner ? (
                                <img
                                    src={
                                        bannerFile[0]?.preview ??
                                        subhiroValues.banner
                ...
                <div className="flex justify-end items-center gap-3 pt-5">
                    ...
                    {formStage === 4 && (
                        <RoundedButton
                            className={`!px-6 !bg-[dodgerBlue] hover:!bg-blue-600 !text-white disabled:!bg-neutral-300 disabled:!text-neutral-500 disabled:pointer-events-none`}
                            disabled={isUploading || isUpdateSubhiroLoading}
                            type="submit"
                        >
                            {isUploading || isUpdateSubhiroLoading ? (
                                <Spinner size="md" />
                            ) : (
                                "Update"
                            )}
